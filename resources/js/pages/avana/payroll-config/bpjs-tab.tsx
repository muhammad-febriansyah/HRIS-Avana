...
import { flattenProgram, SECTIONS } from './types';
import type { BpjsProgram, FlatRecord } from './types';
...
export default function BpjsTab({ programs }: { programs: BpjsProgram[] }) {
    ...
    const rows: FlatRecord[] = programs.map(flattenProgram);
    ...
    const deleteRecord = () => {
        ...
        router.delete(
            PayrollConfigController.destroyBpjsProgram(confirm.id).url,
            {
                preserveScroll: true,
