...
import { claimTypeLabel, statusLabel } from './types';

/* ---------- shared field styles (mirror benefit module) ---------- */
...
/** Colored badge describing a claim status. */
export function StatusBadge({ status }: { status: string }) {
    const palette: Record<string, [string, string]> = {
        pending: [C.amber, 'rgba(217,119,6,.1)'],
        approved: [C.green, 'rgba(22,163,74,.1)'],
        rejected: [C.red, 'rgba(220,38,38,.1)'],
        paid: [C.primary, 'rgba(47,84,201,.1)'],
    };
    const [color, bg] = palette[status] ?? [C.muted, 'rgba(107,114,128,.12)'];
...
/** Colored chip describing a claim type. */
