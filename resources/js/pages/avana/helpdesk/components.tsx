...
/** Color tokens per ticket priority. */
const PRIORITY_COLORS: Record<string, [string, string]> = {
    low: [C.muted, 'rgba(107,114,128,.12)'],
    medium: [C.primary, 'rgba(47,84,201,.1)'],
    high: [C.amber, 'rgba(217,119,6,.1)'],
    urgent: [C.red, 'rgba(220,38,38,.1)'],
};

/** Colored badge describing a ticket priority. */
export function PriorityBadge({ priority }: { priority: string }) {
    const [color, bg] = PRIORITY_COLORS[priority] ?? PRIORITY_COLORS.medium;
...
/** Color tokens per ticket status. */
const STATUS_COLORS: Record<string, [string, string]> = {
    open: [C.primary, 'rgba(47,84,201,.1)'],
    in_progress: [C.amber, 'rgba(217,119,6,.1)'],
    resolved: [C.green, 'rgba(22,163,74,.1)'],
    closed: [C.muted, 'rgba(107,114,128,.12)'],
};

/** Colored pill describing a ticket workflow status. */
