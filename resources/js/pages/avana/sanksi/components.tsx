...
/* ---------- shared field styles (mirror benefit-form) ---------- */
...
/** Colored pill describing a violation type. */
...
/** Colored pill describing the penalty type (warning vs deduction). */
export function PenaltyBadge({ type }: { type: string }) {
    const deduction = type === 'deduction';
    const color = deduction ? C.red : C.amber;
    const bg = deduction ? 'rgba(220,38,38,.1)' : 'rgba(217,119,6,.1)';
